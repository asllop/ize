...
use crate::{ast::{Expr, Literal, ExprSet}, IzeErr, lexer::{TokenKind::{self, *}, TokenStream}, Pos, BuildErr};
...
impl<'a> Grammar<'a> {
    ...
    pub fn parse(&mut self, grammar: &'static [Elem]) -> GrammarCollector {
        ...
        GrammarCollector { result, token_stream: self.token_stream }
    }
            ...
            match elem {
                Elem::Tk(token_kind) => {
                    if self.token_stream.is_token(*token_kind, 0) {
                        let (_, pos) = self.token_stream.consume_token().into_particle().expect("The stream should contain a particle");
                ...
                Elem::OrTk(tokens) => {
                    if self.token_stream.check_tokens(*tokens, 0) {
                        let (token_kind, pos) = self.token_stream.consume_token().into_particle().expect("The stream should contain a particle");
                        result.atoms.push(ResultAtom::Token {
                            pos,
                            val: token_kind
                        });
                ...
                Elem::Identifier => {
                    if self.token_stream.is_ident(0) {
                        let (id, pos) = self.token_stream.consume_token().into_ident().expect("The stream should contain an identifier");
                        result.atoms.push(ResultAtom::Ident {
                            pos,
                            val: id
                        });
                    } else {
                ...
                Elem::Literal => {
                    if self.token_stream.is_literal(0) {
                        let (lit, pos) = self.token_stream.consume_token().into_literal().expect("The stream should contain a literal");
                        match lit {
                            Literal::Integer(i) => {
                                result.atoms.push(ResultAtom::IntLiteral {
                                    pos,
                                    val: i
                                });
                            },
                            Literal::Float(f) => {
                                result.atoms.push(ResultAtom::FltLiteral {
                                    pos,
                                    val: f
                                });
                            },
                            Literal::String(s) => {
                                result.atoms.push(ResultAtom::StrLiteral {
                                    pos,
                                    val: s
                                });
                            },
                            Literal::Boolean(b) => {
                                result.atoms.push(ResultAtom::BoolLiteral {
                                    pos,
                                    val: b
                                });
                            },
                            Literal::None => {
                                result.atoms.push(ResultAtom::NoneLiteral {
                                    pos,
                                });
                            },
                            Literal::Null => {
                                result.atoms.push(ResultAtom::NullLiteral {
                                    pos,
                                });
                            },
                        }
                ...
                Elem::Expr(expr_fn) => {
                    match expr_fn(self.token_stream) {
                        Ok(expr) => {
...
impl<'a> GrammarCollector<'a> {
    ...
    pub fn collect(self, block: fn(ParseResult, &mut TokenStream) -> Result<Expr, IzeErr>) -> Result<Expr, IzeErr> {
    ...
    Grammar::new(token_stream).parse(&[
        ...
        if result.succeed {
            ...
            if result.atoms.is_empty() {
                ...
                Err(result.error.expect("Result error can't be None if succeed is false"))
            }
        }
    })
}
...
fn term_expr(token_stream: &mut TokenStream) -> Result<Expr, IzeErr> {
    Grammar::new(token_stream).parse(&[
        ...
        if result.succeed {
            ...
            let expr = Expr::new(ExprSet::Binary {
