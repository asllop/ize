...
fn check_valid_tokens() {
    ...
    assert_eq!(
        tokens.pop(),
        Some(Token::new(TokenPos::new(1, 24, 25, 38), TokenKind::Minus))
    );
    assert_eq!(
        tokens.pop(),
        Some(Token::new(TokenPos::new(1, 23, 24, 37), TokenKind::Plus))
    );
    assert_eq!(
        tokens.pop(),
        Some(Token::new(
            TokenPos::new(1, 22, 23, 36),
...
fn check_invalid_tokens() {
    ...
    assert_eq!(err.pos, TokenPos::new(0, 6, 7, 6));
    ...
    assert!(tokens.is_err());
    let err = tokens.err().unwrap();
    assert_eq!(err.pos, TokenPos::new(0, 4, 16, 4));
}
